...
function searchRole(e) {
    ...
    mainTable.DataTable({
        ajax: {
            ...
            url: '/role/search',
            data: function (d) {
                d.name = $('#role_search').val();
                d.status = $('#role_status_search').val();
            }
        },
        bFilter: false,
        processing: true,
...
function resetTable(event) {
    ...
    var mainTable = $('#main-table-role');
    mainTable.DataTable().clear().destroy();
    document.getElementById('search-role').reset();
    showAllData();
}

$("body").on("click", ".switchStatus", function () {
    var id    = $(this).data("id");
    var check = $("#switchRole" + id).is(":checked");
    var table = $('#main-table-role').DataTable();
